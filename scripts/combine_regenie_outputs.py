...
def combine_outputs(out, final=False):
    ...
    if (final):
        file_pattern = "final_regenie_outputs/*_*_saige_*results_*_*.txt"
        file_single_pattern = "final_regenie_outputs/*_*_saige_*results_*_*.singleAssoc.txt"
    ...
    for file in files:
        ...
        if (final):
            match = re.search(r"final_regenie_outputs/(.*?)_(.*?)_saige_conditioned_results_(.*?)_.*?\.txt", file)
        else:
            match = re.search(r"regenie_outputs/(.*?)_(.*?)_.*?_saige_results_(.*?)\.txt", file)
    ...
    for file in files_single:
        # Extract ancestry, trait, variant class and mode from filename
        if (final):
            match = re.search(r"final_regenie_outputs/(.*?)_(.*?)_saige_conditioned_results_(.*?)_.*?\.txt\.singleAssoc\.txt", file)
        else:
            match = re.search(r"regenie_outputs/(.*?)_(.*?)_.*?_saige_results_(.*?)\.txt\.singleAssoc\.txt", file)
