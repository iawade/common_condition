...
# Check if --condition is a file and read it if so
if [[ -f "$CONDITION" ]]; then
  CONDITION=$(cat "$CONDITION" | tr '\n' ',')
  CONDITION="${CONDITION%,}"  # Remove trailing comma
fi

# Checks for required parameters
if [[ -z "$CONDITION" || -z "$GENE" ]]; then
  echo "Both --condition and --gene must be provided!"
  exit 1
fi
...
## Filter group file if gene is provided
if [[ -n "$GENE" ]]; then
  FILTERED_GROUPFILE="${OUT}_filtered_groupfile.txt"
  grep -E "^$GENE\s" "$GROUPFILE" > "$FILTERED_GROUPFILE"
  GROUPFILE="$FILTERED_GROUPFILE"
fi

# If running variant tests
