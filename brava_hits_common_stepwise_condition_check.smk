...
rule all:
    input:
        ...
        expand("run_files/{gene}_dist{distance}_maf{maf}.vcf.bgz", 
        gene=genes, distance=config["distance"], maf=config["maf"]),
        expand("run_files/{gene}_dist{distance}_maf{maf}.vcf.bgz.csi", 
        gene=genes, distance=config["distance"], maf=config["maf"]),
...
rule spa_tests_stepwise_conditional:
    input:
        vcf="run_files/{gene}_dist{distance}_maf{maf}.vcf.bgz",
